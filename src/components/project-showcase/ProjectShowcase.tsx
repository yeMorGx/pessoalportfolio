...
export function ProjectShowcase({ project, locale }: { project: Project; locale: Locale }) {
  const rootRef = useRef<HTMLElement | null>(null);
  const copy = siteCopy[locale].showcase;
  ...
  const features = project.product_features.length ? project.product_features : copy.fallbackFeatures;
  const results = project.product_results.length ? project.product_results : copy.fallbackResults;
  ...
  return (
    <main ref={rootRef} lang={locale === "pt" ? "pt-BR" : "en"} className="min-h-screen overflow-x-hidden bg-ink text-white">
      <section className="relative overflow-hidden border-b border-white/10 px-5 pb-12 pt-6 sm:px-8 sm:pb-16 sm:pt-8">
        ...
        <div className="relative mx-auto max-w-7xl">
          <div data-project-intro className="flex items-center justify-between gap-4 border-b border-white/10 pb-5">
            <Link href={`${homePath(locale)}#projetos`} className="group inline-flex items-center gap-2 text-xs text-slate-400 transition-colors hover:text-white focus:outline-none focus-visible:text-mint">
              <PortfolioIcon name="left" className="h-3 w-3 transition-transform group-hover:-translate-x-0.5" />
              {copy.back}
            </Link>
            <div className="flex items-center gap-3">
              <LanguageSwitch locale={locale} englishHref={projectPath("en", project.slug)} portugueseHref={projectPath("pt", project.slug)} />
              <span className="hidden font-mono text-[0.62rem] uppercase text-steel sm:inline">Case / {project.slug}</span>
            </div>
          </div>
          ...
          <div className="mt-10 grid gap-9 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
            ...
            <div data-project-intro data-project-cover-wrap className="relative aspect-[16/10] overflow-hidden rounded-sm border border-white/12 bg-graphite shadow-2xl shadow-black/30">
              <Image data-project-cover src={project.cover_image_url} alt={`${copy.coverAlt} ${project.title}`} fill priority sizes="(min-width: 1024px) 58vw, 100vw" className="object-cover will-change-transform" />
            </div>
          </div>
          ...
          <div data-project-intro className="mt-9 grid grid-cols-2 border-y border-white/10 sm:grid-cols-3">
            <div className="border-r border-white/10 py-4 pr-4">
              <p className="font-mono text-[0.6rem] uppercase text-steel">{copy.role}</p>
              <p className="mt-1 text-xs text-slate-200">{project.product_role ?? copy.defaultRole}</p>
            </div>
            <div className="py-4 pl-4 sm:border-r sm:border-white/10 sm:px-4">
              <p className="font-mono text-[0.6rem] uppercase text-steel">{copy.technologies}</p>
              <p className="mt-1 text-xs text-slate-200">{project.tech_stack.length} {copy.inSolution}</p>
            </div>
            <div className="col-span-2 border-t border-white/10 py-4 sm:col-span-1 sm:border-t-0 sm:pl-4">
              <p className="font-mono text-[0.6rem] uppercase text-steel">{copy.status}</p>
              <p className="mt-1 flex items-center gap-2 text-xs text-slate-200"><span className="h-1.5 w-1.5 bg-mint" /> {copy.published}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section data-project-section className="border-b border-white/10 px-5 py-10 sm:px-8 sm:py-16">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.25fr_0.75fr]">
          <div data-project-reveal>
            <SectionLabel number="01">{copy.overview}</SectionLabel>
            <h2 className="mt-5 max-w-2xl font-display text-2xl font-semibold leading-snug text-ceramic sm:text-3xl">{copy.resolves}</h2>
          ...
          <div data-project-reveal className="border-t border-white/10 pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
            <p className="font-mono text-[0.65rem] uppercase text-steel">{copy.stack}</p>
      ...
      {project.video_url ? (
        <section data-project-section className="border-b border-white/10 px-5 py-14 sm:px-8 sm:py-16">
          <div className="mx-auto max-w-7xl">
            <div data-project-reveal>
              <SectionLabel number="02">{copy.demo}</SectionLabel>
              <h2 className="mt-5 font-display text-2xl font-semibold text-ceramic sm:text-3xl">{copy.moving}</h2>
            </div>
      ...
      <section data-project-section className="border-b border-white/10 px-5 py-14 sm:px-8 sm:py-16">
        <div className="mx-auto max-w-7xl">
          <div data-project-reveal>
            <SectionLabel number={project.video_url ? "03" : "02"}>{copy.gallery}</SectionLabel>
            <h2 className="mt-5 font-display text-2xl font-semibold text-ceramic sm:text-3xl">{copy.galleryHeading}</h2>
          </div>
      ...
      <section data-project-section className="px-5 py-14 sm:px-8 sm:py-16">
        <div className="mx-auto grid max-w-7xl gap-12 md:grid-cols-2 md:gap-8">
          <div data-project-reveal>
            <SectionLabel number={project.video_url ? "04" : "03"}>{copy.features}</SectionLabel>
            <h2 className="mt-5 font-display text-2xl font-semibold text-ceramic">{copy.built}</h2>
          ...
          <div data-project-reveal>
            <SectionLabel number={project.video_url ? "05" : "04"}>{copy.impact}</SectionLabel>
            <h2 className="mt-5 font-display text-2xl font-semibold text-ceramic">{copy.value}</h2>
            <ol className="mt-6 border-t border-white/10">
      ...
      <footer data-project-section className="border-t border-white/10 px-5 py-10 sm:px-8">
        <div data-project-reveal className="mx-auto flex max-w-7xl flex-col justify-between gap-5 sm:flex-row sm:items-center">
          <div>
            <p className="font-mono text-[0.62rem] uppercase text-steel">{copy.next}</p>
            <p className="mt-2 text-base font-medium text-white">{copy.nextBody}</p>
          </div>
          <Link href={projectsPath(locale)} className="group inline-flex h-11 items-center gap-2 self-start rounded-sm border border-white/15 px-4 text-sm text-white transition-colors hover:border-mint/60 sm:self-auto">
            {copy.all}
            <PortfolioIcon name="link" className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </Link>
        </div>
