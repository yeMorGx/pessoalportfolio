...
export function ProjectCard({ project, index, locale }: ProjectCardProps) {
  ...
  return (
    <Link href={projectPath(locale, project.slug)} className="group block border-b border-white/12 py-9 focus:outline-none focus-visible:ring-2 focus-visible:ring-mint sm:py-12">
      <article data-reveal className="grid gap-7 lg:grid-cols-12 lg:items-center lg:gap-12">
        <div className={`${reverse ? "lg:order-2" : ""} lg:col-span-7`}>
          <div ref={mediaRef} className={`relative aspect-[16/10] overflow-hidden border border-white/12 bg-graphite shadow-2xl shadow-black/25 will-change-transform ${isFullscreenCover ? "" : "p-5 sm:p-8"}`}>
            {isFullscreenCover ? (
              <Image src={project.cover_image_url} alt={`${copy.coverAlt} ${project.title}`} fill sizes="(min-width: 1024px) 58vw, 100vw" className="object-cover transition duration-700 group-hover:scale-[1.025]" />
            ) : (
              <div className="relative h-full w-full overflow-hidden border border-white/10 bg-black/20 shadow-xl shadow-black/25">
                <Image src={project.cover_image_url} alt={`${copy.coverAlt} ${project.title}`} fill sizes="(min-width: 1024px) 48vw, 90vw" className="object-cover transition duration-700 group-hover:scale-[1.025]" />
              </div>
            )}
        ...
        <div className={`${reverse ? "lg:order-1" : ""} lg:col-span-5`}>
          <div className="flex items-center justify-between gap-4 border-b border-white/12 pb-3 font-mono text-[0.6rem] uppercase text-steel">
            <span>{project.product_role ?? copy.defaultRole}</span>
