...
export function ProjectsArchive({ projects, locale }: ProjectsArchiveProps) {
  const orderedProjects = [...projects].sort((a, b) => a.order - b.order);
  const copy = siteCopy[locale].archive;
  ...
  return (
    <main lang={locale === "pt" ? "pt-BR" : "en"} className="min-h-screen bg-ink text-ceramic">
      <header className="fixed left-0 right-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-4">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between border border-white/10 bg-ink/[0.9] px-3 shadow-[0_14px_40px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:px-4">
          <Link href={homePath(locale)} className="flex h-full items-center gap-3 text-sm font-semibold focus:outline-none focus-visible:text-mint">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <LanguageSwitch locale={locale} englishHref="/projects" portugueseHref="/pt/projetos" />
            <Link href={homePath(locale)} className="inline-flex h-9 items-center gap-2 border border-white/15 px-3 text-xs text-smoke transition-colors hover:border-mint/60 hover:text-mint focus:outline-none focus-visible:ring-2 focus-visible:ring-mint">
              <ArrowLeft size={14} />
              <span className="hidden min-[430px]:inline">{copy.home}</span>
            </Link>
          </div>
        </div>
      </header>
      ...
      <section ref={stageRef} className="projects-archive-stage relative min-h-screen overflow-hidden border-b border-white/10 bg-ink">
        ...
        <div ref={railRef} className="projects-archive-rail relative flex flex-col gap-16 px-5 pb-20 pt-28 sm:px-8 lg:h-screen lg:w-max lg:flex-row lg:items-center lg:gap-[11vw] lg:px-[7vw] lg:pb-0 lg:pt-0">
          <div className="w-full max-w-2xl shrink-0 lg:w-[28rem]">
            ...
            <p className="mt-8 font-mono text-[0.62rem] uppercase text-mint">{copy.eyebrow}</p>
            <h1 className="mt-5 font-display text-5xl font-semibold leading-[0.92] text-ceramic sm:text-6xl lg:text-7xl">{copy.heading}</h1>
            <p className="mt-6 max-w-md text-base leading-7 text-steel">{copy.body}</p>
          ...
          {orderedProjects.map((project, index) => {
            ...
            return (
              <article key={project.id} data-archive-project={index} className={`projects-archive-item w-full scroll-mt-24 lg:w-[min(68vw,62rem)] lg:shrink-0 ${verticalOffsets[index % verticalOffsets.length]}`}>
                <Link href={projectPath(locale, project.slug)} className="group block focus:outline-none focus-visible:ring-2 focus-visible:ring-mint">
                  <div className="grid gap-5 border-y border-white/12 py-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.65fr)] lg:items-center lg:gap-8">
                    <div className={`relative aspect-[16/10] overflow-hidden border border-white/12 bg-graphite shadow-2xl shadow-black/30 ${project.cover_display === "fullscreen" ? "" : "p-4 sm:p-6"}`}>
                      <div className={`relative h-full w-full overflow-hidden ${project.cover_display === "fullscreen" ? "" : "border border-white/10 bg-black/20"}`}>
                        <Image src={project.cover_image_url} alt={`${projectCopy.coverAlt} ${project.title}`} fill priority={index === 0} sizes="(min-width: 1024px) 46vw, 100vw" className="object-cover transition duration-700 group-hover:scale-[1.025]" />
                      </div>
          ...
          <div className="w-full max-w-xl shrink-0 border-y border-white/12 py-8 lg:w-[26rem]">
            <p className="font-mono text-[0.6rem] uppercase text-coral">{copy.next}</p>
            <h2 className="mt-5 font-display text-4xl font-semibold leading-tight">{copy.nextHeading}</h2>
            <p className="mt-5 text-sm leading-6 text-steel">{copy.nextBody}</p>
      ...
      <div className="projects-archive-controls pointer-events-none fixed bottom-5 left-1/2 z-40 hidden w-[min(92vw,56rem)] -translate-x-1/2 items-center gap-4 lg:flex">
        ...
        <div className="pointer-events-auto flex border border-white/15 bg-ink/90 backdrop-blur-xl">
          <button type="button" onClick={() => scrollToProject(activeIndex - 1)} disabled={activeIndex <= 0} aria-label={copy.previous} title={copy.previous} className="inline-flex h-10 w-10 items-center justify-center border-r border-white/15 text-ceramic transition-colors hover:text-mint focus:outline-none focus-visible:ring-2 focus-visible:ring-mint disabled:cursor-not-allowed disabled:text-slate-700">
            <ChevronLeft size={17} />
          </button>
          <button type="button" onClick={() => scrollToProject(activeIndex + 1)} disabled={activeIndex >= orderedProjects.length - 1} aria-label={copy.following} title={copy.following} className="inline-flex h-10 w-10 items-center justify-center text-ceramic transition-colors hover:text-mint focus:outline-none focus-visible:ring-2 focus-visible:ring-mint disabled:cursor-not-allowed disabled:text-slate-700">
